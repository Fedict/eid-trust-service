...
public class ConfigurationServlet extends HttpServlet {
	...
	private static final Log LOG = LogFactory
			.getLog(ConfigurationServlet.class);
	...
	private String getName(HttpServletRequest request) throws ServletException {
		...
		while (paramNames.hasMoreElements()) {
			...
			} else if (!paramName.equals(CA_ROOT_FIELD)
					&& paramName.startsWith(CA_ROOT_FIELD)) {
				return paramName.substring(paramName.indexOf(CA_ROOT_FIELD)
						+ CA_ROOT_FIELD.length());
			}
		}
	...
	private void addEditCAMarkup(PrintWriter out, CAConfiguration ca) {
		...
		addDisabledTextInput(out, CA_NAME_LABEL, CA_NAME_FIELD + ca.getName(),
				ca.getName());
		addTextInput(out, CA_CRL_RECORD_LABEL,
				CA_CRL_RECORDS_FIELD + ca.getName(),
				Long.toString(ca.getCrlRecords()));
		if (null == ca.getRoot()) {
			addTextInput(out, CA_CRL_REFRESH_LABEL,
					CA_CRL_REFRESH_FIELD + ca.getName(),
		...
		if (null == ca.getRoot() && null != ca.getCertificate()) {
			addLink(out, CertificateServlet.getPath(ca.getName()),
					"Certificate");
		}
	...
	private void addDisabledTextInput(PrintWriter out, String label,
	...
	private void addSubmit(PrintWriter out, Action action) {
		out.println("<input name=\"" + ACTION + "\" type=\"submit\" value=\""
				+ action + "\"/>");
	}
